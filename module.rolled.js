...
function mousedown(e) {
    ...
    if (e.timeStamp < timeStamp + config$2.simulatedEventDelay * 1000) { return; }
    timeStamp = undefined;
    updateClass(config$2.mouseClass);
}

function keydown(e) {
    // If key is not tab, enter or escape do nothing
    if ([9, 13, 27].indexOf(e.keyCode) === -1) { return; }
    updateClass(config$2.keyClass);
}

function touchend(e) {
    timeStamp = e.timeStamp;
    updateClass(config$2.touchClass);
}

on$2(document, 'mousedown', mousedown);
on$2(document, 'keydown', keydown);
on$2(document, 'touchend', touchend);
...
Sparky.fn('lines', function(node) {
    return Stream$1.of(lines);
});

Sparky.fn('lines-actions', delegate$2({
    'click': {
...
Sparky.fn('breakpoints', function (node) {
    return Stream$1.of(breakpoints);
});
...
Sparky.fn('breakpoints-actions', delegate$2({
    'click': {
        'button[name]': overload(get$1('name'), {
